...
import numpy as np
from utils import load_tiles_dataset_from_cache, TilesDataset
...
class TileSegmenter:
    ...
    def summaries(self):
        if self._summaries is None:
            ...
            bg_pred_image_rounded = tf.expand_dims(tf.gather(tf.transpose(bg_pred_image_rounded, [3, 0, 1, 2]), 0),
                                                   axis=3)

            tf.summary.image('BACKGROUND (target)', bg_target_image, max_outputs=how_many_images)
            ...
            fg_pred_image_rounded = tf.expand_dims(tf.gather(tf.transpose(fg_pred_image_rounded, [3, 0, 1, 2]), 1),
                                                   axis=3)

            tf.summary.image('FOREGROUND (target)', fg_target_image, max_outputs=how_many_images)
            ...
            # --- merge all summaries and initialize the summery writer
...
if __name__ == '__main__':
    # Load TILES data
    tiles_dataset = load_tiles_dataset_from_cache('data/tiles_protocol_03.pickle')
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(batches_each_epoch):
            ...
            for _ in range(batches_each_epoch):
                ...
                _, curr_loss = sess.run(fetches=[model.train_step, model.loss],
