...
from utils import get_mnist_data, EPS
...
def tiny_convnet(x, keep_prob):
    with tf.variable_scope('tiny_cnn'):
        # Restore original shape fot 'x' (it is loaded flat)
        ...
        probabilities = dense(h_flat, units=num_classes, activation=softmax)
    ...
    with tf.Session() as sess:
        ...
        for _ in range(num_test_batches):
            ...
            test_accuracy = sess.run(fetches=accuracy,
                                     feed_dict={x: mnist.test.images, targets: target_batch, p: 1.})
        print('*' * 50)
