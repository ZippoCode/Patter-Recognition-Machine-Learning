...
from utils import EPS, get_mnist_data
...
def single_layer_net(x):
    ...
    with tf.variable_scope('single_layer_net'):
        W = tf.Variable(initial_value=tf.random_normal(shape=[input_dim, num_classes]), name='weights')
        b = tf.Variable(initial_value=tf.zeros(shape=[num_classes]), name='biases')
...
def multi_layer_net(x):
    ...
    with tf.variable_scope('multi_layer_net'):
        ...
        W_2 = tf.Variable(initial_value=tf.random_normal(shape=[hidden_dim, num_classes]), name='l2_weights')
        b_2 = tf.Variable(initial_value=tf.zeros(shape=[num_classes]), name='l2_biases')
...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        ...
        # Eventually evaluate on whole test set when training size
