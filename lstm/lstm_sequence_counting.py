...
class DeepCounter:
    ...
    def __init__(self, x, targets, args):
        ...
        self.num_classes = targets.get_shape()[-1]
    ...
    def inference(self):
        if self._inference is None:
            ...
            self._inference = tf.layers.dense(last_output, units=self.num_classes, activation=tf.nn.softmax)
    ...
    def accuracy(self):
        if self._accuracy is None:
            correct_prediction = tf.equal(tf.round(self.inference), targets)
...
if __name__ == '__main__':
    ...
    synchetic_dataset = SyntheticSequenceDataset()
    ...
    with tf.Session() as sess:
        ...
        # Load dataset
        train_data, train_targets, test_data, test_target = synchetic_dataset.data
        ...
        for epoch in range(0, training_epochs):
            ...
            for _ in range(0, batches_each_epoch):
                ...
                curr_loss, _ = sess.run(fetches=[deep_counter.loss, deep_counter.train_step],
                                        feed_dict={data: data_batch, targets: target_batch})
                loss_current_epoch.append(curr_loss)

                # Update data pointer
                start_idx += batch_size

            print(
                'Epoch {:02d} - Loss on train set: {:02f}'.format(epoch, sum(loss_current_epoch) / batches_each_epoch))
        ...
        for _ in range(0, num_test_batches):
            end_idx = start_idx + batch_size
            data_batch, target_batch = test_data[start_idx, end_idx], test_target[start_idx, end_idx]
