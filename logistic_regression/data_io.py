...
def gaussians_dataset(num_gaussian, num_points, mus, stds):
    """
    Provides a data set made by several gaussians

    :param num_gaussian:  int
        The number of desired gaussian components
    :param num_points: list
        A list of cardinality of points (one for each gaussians)
    :param mus: list
        A list of means (one for each gaussians, e.g. [[1,1], [3,1])
    :param stds: list
        A list of stds (one per each gaussian, e.g. [[1,1], [2,2]).
    :return:
    """

    assert num_gaussian == len(mus) == len(stds) == len(num_points)
    ...
    for i in range(0, num_gaussian):
        ...
        # It takes the list and places it along the diagonal
        cov = np.diag(std)

        X.append(np.random.multivariate_normal(mean=mu, cov=cov, size=2 * num_pt))
        Y.append(np.ones(shape=2 * num_pt) * i)
