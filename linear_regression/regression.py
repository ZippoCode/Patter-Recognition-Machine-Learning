    ...
    with tf.Session() as sess:
        ...
        for i in range(100):
            ...
            for bo_w, br_w in zip(body_weight, brain_weight):
                # Session runs train_op and fetch value of loss
                _, l = sess.run([train_step, loss], feed_dict={x: bo_w, y: br_w})
                total_loss += l
            print('Epoch {0}: {1}'.format(i, total_loss / num_examples))

            # Plot current results
            plt.cla()  # clean exis first
            w_value, b_value = sess.run([w, b])  # get current numeric solution
